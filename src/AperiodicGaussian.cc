...
#include <config.h>
#endif
#include "AperiodicGaussian.h"
#include <iostream>

AperiodicGaussian::AperiodicGaussian(const double a, const double d, const int n)
  : a(a), d(d), grid(n+1), dx(d/n), dxInv(1.0/dx) {
  grid=0.0;
  ...
  for (int iImage=-nImage; iImage<=nImage; ++ iImage) {
    for (int i=0; i<=n; ++i) {
      double x=i*dx;
      double expax2 = exp(-a*pow((x-iImage*d),2))*((iImage%2==0)?1:-1);
      grid(i)[0]+= expax2;
...
/*int main(int argc, char** argv) {
  std::cout << "#Aperiodic gausian test" << std::endl;
  AperiodicGaussian pg(0.01,10,3);
  for (int i=0; i<=100; ++i) {
