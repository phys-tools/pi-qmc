...
class H5Lib {
public:
    static hid_t createScalarDataset(int nstep, hid_t writingGroupID,
            std::string name);
    ...
    static void writeTypeString(const std::string& typeString,
            hid_t& dataSetID);
    static hid_t createScalarInH5File(const ScalarEstimator& est,
            hid_t containerID, int nstep);
    static hid_t createGroupInH5File(const std::string& name,
            hid_t containerID);
};
