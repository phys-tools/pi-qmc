...
public:
  Loop(const int nrepeat, const int totalSimTime, const std::string timer,
      ...
      CompositeAlgorithm(nsteps), nrepeat(nrepeat), totalSimTime(totalSimTime), timer(
          timer), mpi(mpi) {
  }
  ...
  void printElapsedTime(double dif) {
    ...
    int mn = ((d) % 3600) / 60;
    int sc = ((d) % 3600) % 60;
    std::cout << "Time elapsed from start of simulation :: " << hr
        << " hour(s), " << mn << " min(s), " << sc
        << " sec(s), or total time in seconds :: " << dif << " secs."
        << std::endl << std::flush;
  }
  ...
  void printAlgorithmTime(double dt) {
    ...
    int mn = d / 60;
    int sc = (d) % 60;
    std::cout << "Time spent inside loop [" << timer << "] ..... :: " << mn
        << " min(s), " << sc << " sec(s), or total time in seconds :: " << dt
        << " secs." << std::endl << std::flush;
  }
