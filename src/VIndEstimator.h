...
#ifndef __VIndEstimator_h_
#define __VIndEstimator_h_
...
public:
  ...
  /// Constructor.
  VIndEstimator(const SimulationInfo& simInfo, const CoulombAction*,
    const int nfreq, const int nbin, const int nstride,
    MPIManager *mpi);
  /// Virtual destructor.
  virtual ~VIndEstimator();
...
private:
  ...
  ///
  const int npart, nslice, nfreq, nbin, nstride;
