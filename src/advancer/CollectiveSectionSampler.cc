...
CollectiveSectionSampler::CollectiveSectionSampler(int npart,
        ...
        SuperCell *cell) :
        nlevel(sectionChooser.getNLevel()), npart(npart), sectionBeads(
                &sectionChooser.getBeads()), movingBeads(
                beadFactory.getNewBeads(npart, sectionBeads->getNSlice())), movingIndex(
                new IArray(npart)), identityIndex(npart), sectionChooser(
                sectionChooser), cell(cell), action(action), nrepeat(nrepeat), maximumMovingCount(
                npart), mover(mover), accRejEst(0) {
