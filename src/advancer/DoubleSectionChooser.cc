...
DoubleSectionChooser::DoubleSectionChooser(int nlevel, int npart, Paths &paths,
    ...
    SectionChooser(nlevel, npart, paths, action, beadFactory), doubleAction(
        ...
        beadFactory.getNewBeads(npart, (1 << nlevel) + 1)), permutation1(
        SectionChooser::permutation), permutation2(new Permutation(npart)) {
}
...
void DoubleSectionChooser::run() {
  PROFILE_BEGIN (DoubleSectionChooser);
  ...
  PROFILE_BEGIN (DoubleSectionChooser_initializeAction);
