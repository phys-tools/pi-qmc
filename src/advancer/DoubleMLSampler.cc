...
void DoubleMLSampler::run() {
    ...
    for (int irepeat = 0; irepeat < nrepeat; ++irepeat) {
        ...
        if (isNewPerm) {
            ...
            sectionBeads1->copySlice(*movingIndex1, 0, *movingBeads1,
                    identityIndex, 0);

            sectionBeads1->copySlice(pMovingIndex, nsectionSlice - 1,
                    *movingBeads1, identityIndex, nsectionSlice - 1);
            if (samplingBoth) {
                sectionBeads2->copySlice(*movingIndex2, 0, *movingBeads2,
                        identityIndex, 0);
            ...
            if (RandomNumGenerator::getRand() > acceptProb) {
                ...
                for (int i = 0; i < nmoving; ++i) {
                    for (int islice = 0; islice < sectionBeads->getNSlice();
                            ++islice) {
                        (*rejectedBeads1)(i, islice) = (*movingBeads1)(i,
                                islice);
                        (*rejectedBeads2)(i, islice) = (*movingBeads2)(i,
                                islice);
                    }
                }
                ...
                if (samplingBoth) {
                    ...
                    deltaAction +=
                            (action == 0) ?
                                    0 :
                                    action->getActionDifference(*this, ilevel);
                    activateSection(1);
                }
                deltaAction += doubleAction->getActionDifference(*this, ilevel);
